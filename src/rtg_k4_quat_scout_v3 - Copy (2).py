...
def main():
    ...
    # -------------------------------
    # Double-NS proximity + selection of pairs for post runs
    # -------------------------------
    keys = sorted(Kstars.keys())
    selected_pairs: List[int] = []

    if len(keys) == 1:
        # Only one NS crossing
        j = keys[0]
        selected_pairs = [j]
        print("Single-pair NS found.")
    else:
        # Find closest pair in K
        best_pair = None  # (dK, idx_a, idx_b)
        for ia_idx in range(len(keys)):
            for ib_idx in range(ia_idx + 1, len(keys)):
                ia = keys[ia_idx]
                ib = keys[ib_idx]
                dK = abs(Kstars[ia][0] - Kstars[ib][0])
                if (best_pair is None) or (dK < best_pair[0]):
                    best_pair = (dK, ia, ib)

        dK, ia, ib = best_pair
        if dK <= p.near2:
            print(
                f"Double-NS candidate: "
                f"pair{ia+1}@K≈{Kstars[ia][0]:.6f} and "
                f"pair{ib+1}@K≈{Kstars[ib][0]:.6f} "
                f"(|ΔK|={dK:.6f} ≤ near2)"
            )
        else:
            print(
                f"{len(Kstars)} per-pair crossings found; "
                f"closest are pair{ia+1}@K≈{Kstars[ia][0]:.6f} and "
                f"pair{ib+1}@K≈{Kstars[ib][0]:.6f} "
                f"(|ΔK|={dK:.6f} > near2={p.near2:.3f})."
            )
        selected_pairs = [ia, ib]

    # Optional post run: choose K* from the selected pair(s) then +deltaK
    if p.post_ns:
        k_pick = max(Kstars[j][0] for j in selected_pairs)
        diag = post_ns_demo(k_pick, p, g, outdir, p.deltaK, p.T_diag, p.burn_diag, p.noise)
        print(f"[post] run at K={diag['K_run']:.6f} | r_mean={diag['r_mean']:.6f} | "
              f"r_std={diag['r_std']:.3e} | f_peak_per_step={diag['f_peak_per_step']:.6f}")
        with open(os.path.join(outdir, "k4_post_diag.json"), "w", encoding="utf-8") as f:
            json.dump(diag, f, indent=2)

    if p.lyap_demo:
        k_pick = max(Kstars[j][0] for j in selected_pairs)
        les = lyap_qr(k_pick + p.deltaK, p, g,
