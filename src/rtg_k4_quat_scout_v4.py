# rtg_k4_quat_scout_v4.py
#
# K4 scout: scan all complex pairs, detect per-pair NS, test for double-NS
# proximity, optional post-NS demo & Lyapunov. Topologies: ringdiag, quat_sym.
#
# NEW vs your v3:
#   - Save detailed K* information for each NS pair in k4_kstars.json
#   - Record linear “winding ratio” between the two closest NS pairs
#   - Inject that info into k4_post_diag.json so post-processing has access
...
def eig_pairs(J: np.ndarray, n: int) -> List[complex]:
    ...
    We project out the global phase/velocity using the gauge-free basis, then
    take eigenvalues of the reduced Jacobian. For each complex pair, we keep
    the eigenvalue with Im >= 0.
    """
...
def find_pair_bracket(
    ...
    for i in range(K.size - 1):
        ok_i = (ang[i] > ang_tol) and (abs(ang[i] - math.pi) > ang_tol)
        ok_j = (ang[i + 1] > ang_tol) and (abs(ang[i + 1] - math.pi) > ang_tol)
        if ok_i and ok_j:
...
# -------------------------------
# Helper: pack K* info for JSON
# -------------------------------

def build_kstars_payload(
    Kstars: Dict[int, Tuple[float, float, float]],
    best: Optional[Tuple[float, Tuple[int, int]]]
) -> Dict[str, object]:
    """
    Convert the per-pair K* data + best-pair info into a JSON-serializable
    dictionary:

        {
          "pairs": {
            "pair1": { "pair_index": 1, "K_star": ..., "rho": ..., "angle_rad": ..., "freq_cycles_per_step": ... },
            ...
          },
          "double_ns_candidate": {
             "pairA": 1,
    ...
    """
    payload: Dict[str, object] = {"pairs": {}, "double_ns_candidate": None}

    # Per-pair info
    for j, (kstar, rstar, astar) in Kstars.items():
        pair_id = j + 1  # human-readable
        freq = astar / (2.0 * math.pi)  # cycles per step (linearized)
        payload["pairs"][f"pair{pair_id}"] = {
            "pair_index": pair_id,
            "K_star": float(kstar),
            "rho": float(rstar),
    ...
    # Closest pair info (if we found at least two)
    if best is not None:
        deltaK, (pA, pB) = best
        # back to 0-based keys
        kA, _, aA = Kstars[pA - 1]
        kB, _, aB = Kstars[pB - 1]
        winding = float("nan")
        if abs(aB) > 0:
        ...
        payload["double_ns_candidate"] = {
            "pairA": int(pA),
            "pairB": int(pB),
            "K_star_A": float(kA),
            "K_star_B": float(kB),
            "deltaK": float(abs(kA - kB)),
            "winding_ratio_lin": winding,
        }
...
def run_single_sigma2(p: K4Params, outdir: str) -> None:
    ...
    # Find closest pair (for potential double-NS)
    keys = sorted(Kstars.keys())
    best: Optional[Tuple[float, Tuple[int, int]]] = None  # (deltaK, (pairA, pairB))
    if len(keys) >= 2:
        for a_idx in range(len(keys)):
            for b_idx in range(a_idx + 1, len(keys)):
                da = abs(Kstars[keys[a_idx]][0] - Kstars[keys[b_idx]][0])
                pairnames = (keys[a_idx] + 1, keys[b_idx] + 1)  # 1-based
    ...
    if best is not None and best[0] <= p.near2:
        ...
        kB = Kstars[pB - 1][0]
        print(
            f"Double-NS candidate: pair{pA}@K≈{kA:.6f} and "
            f"pair{pB}@K≈{kB:.6f} (|ΔK|={abs(kA-kB):.6f} ≤ near2)"
        )
    else:
        if len(Kstars) >= 2 and best is not None:
            pA, pB = best[1]
            print(
                f"Two per-pair crossings found, but |ΔK| = "
                f"{best[0]:.6f} > near2 (pairs {pA} & {pB})."
            )
        else:
            print("Single-pair NS found.")

    # Save detailed K* summary (including linear winding estimate)
    kstars_payload = build_kstars_payload(Kstars, best)
    ...
    if p.post_ns:
        ...
        # augment diag with linear info about the NS pairs
        diag["kstars_path"] = kstars_path
        diag["Kstars"] = kstars_payload
        if kstars_payload.get("double_ns_candidate") is not None:
            diag["winding_lin"] = kstars_payload["double_ns_candidate"]["winding_ratio_lin"]
...
def main():
    ...
    if do_sigma2_sweep:
        ...
        # Single-sigma2 mode (backwards compatible)
