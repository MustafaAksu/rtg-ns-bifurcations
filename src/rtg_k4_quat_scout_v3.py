...
# optional sigma2 sweep, post-NS demo & Lyapunov. Topologies: ringdiag, quat_sym.
...
class K4Params(Params):
    ...
    # reuse from Params: post_ns, deltaK, T_diag, burn_diag, noise, lyap_demo, le_q
...
def run_single_sigma2(p: K4Params, outdir: str) -> None:
    ...
    print(
        "Explicit: coupling + triads can push complex pairs to |lambda|=1, "
        "enabling multi-pair NS."
    )
    print(
    ...
    best = None  # (ΔK, (pair_idx1, pair_idx2))
    for a in range(len(keys)):
        for b in range(a + 1, len(keys)):
            da = abs(Kstars[keys[a]][0] - Kstars[keys[b]][0])
            pairnames = (keys[a] + 1, keys[b] + 1)  # 1-based for printing
            if (best is None) or (da < best[0]):
                best = (da, pairnames)

    # Report & choose k_pick for post run:
    has_double = False
    if best is not None and best[0] <= p.near2:
        ...
        # IMPORTANT CHANGE vs v3:
        # Use the *larger* K* of the closest pair (pa, pb),
        # not the max over all pairs. This avoids jumping to a 3rd pair
        # when three crossings exist.
        k_pick = max(kA, kB)
        has_double = True
    else:
        if len(Kstars) >= 2:
            a, b = keys[0], keys[1]
            print(
                "Two per-pair crossings found, but |ΔK| = "
                f"{abs(Kstars[a][0]-Kstars[b][0]):.6f} > near2."
            )
        else:
            print("Single-pair NS found.")
        # Fallback: just take the largest K* among all pairs
    ...
    if p.post_ns:
        ...
        with open(
            os.path.join(outdir, "k4_post_diag.json"),
            "w",
            encoding="utf-8",
        ) as f:
    ...
    if p.lyap_demo:
        # For Lyapunov we follow the usual convention: evaluate at K_pick + deltaK,
        # mirroring the existing post_ns_demo call.
        les = lyap_qr(
            k_pick + p.deltaK,
            p,
            g,
        ...
        with open(
            os.path.join(outdir, "lyap_summary.txt"),
            "w",
            encoding="utf-8",
        ) as f:
...
# -------------------------------
# Main (with optional sigma2 sweep)
# -------------------------------

def main():
    ap = argparse.ArgumentParser(description="K4 double-NS scout (with sigma2 sweep)")
    ...
    ap.add_argument("--sigma2_pts", type=int, default=None)
    ...
    base_outdir = args.outdir or os.path.join(
        f"figs_k4_quat_{args.scheme}_{now_tag()}"
    )
    ensure_dir(base_outdir)

    # Decide whether we are doing a sigma2 sweep or a single run
    do_sweep = (
    ...
    if do_sweep:
        print(
            f"Sigma2 sweep: from {args.sigma2_min:.6f} "
            f"to {args.sigma2_max:.6f} in {args.sigma2_pts} steps.\n"
        )
        sigma_vals = np.linspace(args.sigma2_min, args.sigma2_max, args.sigma2_pts)
        for s in sigma_vals:
            print("\n" + "=" * 72)
            print(f"Running sigma2 = {s:.6f}")
            print("=" * 72 + "\n")
            ...
            outdir = os.path.join(base_outdir, f"sigma2_{s:.6f}")
            run_single_sigma2(p, outdir)
    else:
        p = K4Params(
            scheme=args.scheme,
            dt=args.dt,
        ...
        run_single_sigma2(p, base_outdir)
