...
def run_single_sigma2(p: K4Params, outdir: str) -> None:
    ...
    for a_idx in range(len(keys)):
        for b_idx in range(a_idx + 1, len(keys)):
            ...
            pairnames = (keys[a_idx] + 1, keys[b_idx] + 1)
    ...
    if best is not None and best[0] <= p.near2:
        ...
        print(
            f"Double-NS candidate: pair{best[1][0]}@K≈{kA:.6f} and "
            f"pair{best[1][1]}@K≈{kB:.6f} (|ΔK|={abs(kA-kB):.6f} ≤ near2)"
        )
    else:
        if len(Kstars) >= 2:
            a = keys[0]
